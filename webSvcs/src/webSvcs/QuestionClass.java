...
public class QuestionClass {
	...
	public JSONArray searchlist( ArrayList<String> question)
	{
		...
		else
		{
			...
			{
				if(s.contains("work")||s.contains("start")||s.contains("run"))
				{
					addt=s;
				}
				else
		...
		for(int i=0;i<slist.size();i++)
				{
				  ...
				  Criterion qust=Restrictions.ilike("qus","%"+qus+"%"+addt+"%",MatchMode.ANYWHERE);
				  Criterion desc=Restrictions.ilike("descr","%"+qus+"%"+addt+"%",MatchMode.ANYWHERE);
				...
				qust=Restrictions.ilike("qus","%"+addt+"%"+qus+"%",MatchMode.ANYWHERE);
				desc=Restrictions.ilike("descr","%"+addt+"%"+qus+"%",MatchMode.ANYWHERE);
				...
				if(!result1.isEmpty())
				{
				 ...
				 qust=Restrictions.ilike("qus","%"+qus+"%",MatchMode.ANYWHERE);
				 desc=Restrictions.ilike("descr","%"+qus+"%",MatchMode.ANYWHERE);
				 orExp=Restrictions.or(qust,desc);
				 cr3.add(orExp);
				 cr3.setCacheable(true);
				  result2=cr3.list();
				 if(!result2.isEmpty())
					{
